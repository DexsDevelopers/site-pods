...
<body class="bg-slate-900 text-slate-100">
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-8">
            <div class="mb-8">
                <h1 class="text-3xl font-bold gradient-text">Gerenciar Pedidos</h1>
                <p class="text-slate-400 mt-2">Total de pedidos: <strong><?php echo $total; ?></strong></p>
            </div>
            ...
            <div class="bg-slate-800/50 rounded-xl p-6 mb-6 backdrop-blur-sm border border-purple-800/30">
                <form method="GET" class="flex gap-4 flex-wrap">
                    <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" 
                           ...
                           class="flex-1 min-w-64 px-4 py-2 bg-slate-900/50 border border-purple-800/30 rounded-lg text-slate-100 placeholder-slate-400 focus:outline-none focus:border-purple-600">
                    
                    <select name="status" class="px-4 py-2 bg-slate-900/50 border border-purple-800/30 rounded-lg text-slate-100 focus:outline-none focus:border-purple-600">
                ...
                <table class="w-full">
                    <thead class="bg-slate-900/50 border-b border-purple-800/30">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-purple-400 uppercase">Total</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-purple-400 uppercase">Itens</th>
                        ...
                        <?php else: ?>
                            <?php foreach ($pedidos as $pedido): ?>
                                <tr class="hover:bg-slate-900/30 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div>
                                            <p class="text-sm font-medium"><?php echo htmlspecialchars($pedido['name'] ?? 'Anônimo'); ?></p>
                                            <p class="text-xs text-slate-500"><?php echo htmlspecialchars($pedido['email'] ?? 'N/A'); ?></p>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium">R$ <?php echo number_format($pedido['total_amount'], 2, ',', '.'); ?></td>
                                    <td class="px-6 py-4 text-sm text-slate-400"><?php echo $pedido['total_items']; ?></td>
                                    ...
                                    <td class="px-6 py-4">
                                        <select onchange="updateOrderStatus(<?php echo $pedido['id']; ?>, this.value)" 
                                                class="px-2 py-1 text-xs rounded-lg cursor-pointer <?php 
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex gap-2">
                                            ...
                                            <button onclick="enviarEmail(<?php echo $pedido['id']; ?>, '<?php echo htmlspecialchars($pedido['email']); ?>')" 
                                                    class="p-2 bg-purple-900/30 text-purple-400 rounded-lg hover:bg-purple-900/50 transition">
